...
async function updateData() {
    ...
    const timeout = new Promise((_, reject) => 
        setTimeout(() => reject(new Error('Connection timeout: Make sure your Realtime Database is "Created" in Firebase and Rules are set to true.')), 10000)
    );
    
    try {
        // Race the upload against the timeout
        await Promise.race([dashboardRef.set(data), timeout]);
        markSaved();
        console.log('Data saved to cloud successfully');
    } catch (error) {
        console.error('Error saving to cloud:', error);
        alert(error.message || 'Failed to save to cloud. Check your Firebase configuration.');
        markUnsaved();
    } finally {
        resetUpdateButton();
    }
